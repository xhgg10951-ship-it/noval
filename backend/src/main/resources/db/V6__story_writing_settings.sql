...
-- Idempotent: safe to re-run. MySQL 8.4 / utf8mb4.
-- ============================================================================
...
ALTER TABLE story
    ADD COLUMN IF NOT EXISTS default_target_characters INT NULL DEFAULT 3000
        COMMENT '单章目标字数（Phase 2 章节控制默认 3000）';

-- writing_style: free-text authoring style hint surfaced to the Writer (Phase 8).
ALTER TABLE story
    ADD COLUMN IF NOT EXISTS writing_style VARCHAR(200) NULL
...
ALTER TABLE story
    ADD COLUMN IF NOT EXISTS target_chapter_count INT NULL
